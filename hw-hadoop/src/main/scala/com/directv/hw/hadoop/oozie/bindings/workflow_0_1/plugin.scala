package com.directv.hw.hadoop.oozie.bindings.workflow_0_1

import java.io.InputStream
import java.lang.reflect.Type

import com.directv.hw.core.exception.DapException
import com.directv.hw.hadoop.oozie.bindings.workflow_0_1.WorkflowSerializerCommon._
...
object pluginDescription {
  val jsonSchemaPath = "oozie/metadata/workflow_0_1/jsonSchema.json"
...
import com.directv.hw.hadoop.oozie.bindings.workflow_0_1.pluginDescription.jsonObject2extJsonHas
...
class MapReduceDeserializer() extends JsonDeserializer[MAPREDUCE] {
  ...
  override def deserialize(mapreduceJson: JsonElement, typeOfT: Type, context: JsonDeserializationContext) = {
    ...
    if (json != null) {
      ...
      if (json.has("job-xml")) mapReduce.setJobXml(json.get("job-xml").getAsString)
...
class PigDeserializer() extends JsonDeserializer[PIG] {
  ...
  override def deserialize(pigJson: JsonElement, typeOfT: Type, context: JsonDeserializationContext) = {
    ...
    val pig = new PIG
    if (json != null) {
      pig.setJobTracker(json.get("job-tracker").getAsString)
      pig.setNameNode(json.get("name-node").getAsString)
      if (json.has("prepare")) {
...
class ActionDeserializer() extends JsonDeserializer[ACTION] {
  ...
  override def deserialize(actionJson: JsonElement, typeOfT: Type, context: JsonDeserializationContext) = {
    ...
    if (json != null) {
      ...
      if (json.has("ssh")) action.setSsh(context.deserialize(json.get("ssh"), classOf[SSH]))
