package com.directv.hw.hadoop.oozie.bindings.sqoop_action_0_2

import java.io.InputStream
import java.lang.reflect.Type

import com.directv.hw.core.exception.DapException
import com.directv.hw.hadoop.oozie.bindings.sqoop_action_0_2.WorkflowSerializerCommon._
import com.directv.hw.hadoop.oozie.bindings.sqoop_action_0_2.binding.{ACTION, CONFIGURATION, PREPARE}
...
object pluginDescription {
  val jsonSchemaPath = "oozie/metadata/sqoop_action_0_2/jsonSchema.json"
...
import com.directv.hw.hadoop.oozie.bindings.sqoop_action_0_2.pluginDescription._
...
class SqoopDeserializer() extends JsonDeserializer[ACTION] {
  ...
  override def deserialize(sqoopJson: JsonElement, typeOfT: Type, context: JsonDeserializationContext) = {
    ...
    sqoop.setJobTracker(json.get("job-tracker").getAsString)
    sqoop.setNameNode(json.get("name-node").getAsString)
    if (json.has("prepare")) {
      sqoop.setPrepare(context.deserialize(json.get("prepare").getAsJsonObject, classOf[PREPARE]))
      if (sqoop.getPrepare.getDelete.isEmpty && sqoop.getPrepare.getMkdir.isEmpty) sqoop.setPrepare(null)
    }
    if (json.hasStringData("job-xml")) sqoop.setJobXml(json.get("job-xml").getAsString)
