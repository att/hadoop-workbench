...
class GraphToWorkflowConverterImpl_0_1 extends GraphToWorkflowConverterBase with LazyLogging {
  ...
  override lazy val pluginDeserializers: List[(Class[_], Class[_])] =
    com.directv.hw.hadoop.oozie.bindings.shell_action_0_3.pluginDescription.additionalDeserializers ++
      ...
      com.directv.hw.hadoop.oozie.bindings.workflow_0_1.pluginDescription.additionalDeserializers ++
    ...
    graph.nodes foreach { node =>
      ...
      (node.`type`, node.subtype, node.version) match {
        ...
        case (Oozie.types.workflowControl, "fork", getSupportedWorkflowVersion) =>
          ...
          connections withFilter(_.from == node.id) foreach { connection =>
        ...
        case (Oozie.types.action, subtype: String, version: String) =>
