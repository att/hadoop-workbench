...
class WorkflowToGraphConverterImpl_0_2_5 extends WorkflowToGraphConverterBase with LazyLogging {

  // workflow version this converter supports
  override def getSupportedWorkflowVersion: String = "0.2.5"

  import WorkflowConverter.Oozie

  def getJsonSchemaAsString: String = com.directv.hw.hadoop.oozie.bindings.workflow_0_2_5.pluginDescription.getJsonSchemaAsString
  ...
  override lazy val pluginSerializers: List[(Class[_], Class[_])] =
    com.directv.hw.hadoop.oozie.bindings.shell_action_0_3.pluginDescription.additionalSerializers ++
      ...
      com.directv.hw.hadoop.oozie.bindings.workflow_0_2_5.pluginDescription.additionalSerializers ++
  ...
  def extractAction(action: ACTION): (Any, String, String) = {
    Option(action.getMapReduce) map { mapReduce =>
      ...
      val properties = Option(mapReduce.configuration.getProperty) map (_.toList) getOrElse List.empty
  ...
  def getStartPropertiesJson(credentials: CREDENTIALS): String = {
    ...
    if(credentials != null && credentials.getCredential != null && credentials.getCredential.nonEmpty) {
      val gson = new GsonBuilder().create()
      val tree = gson.toJsonTree(credentials, classOf[CREDENTIALS])
      result.add("credential", tree.getAsJsonObject.get("credential"))
    }
  ...
  override protected def getNodeBindings: List[Class[_]] = {
    List(
      classOf[com.directv.hw.hadoop.oozie.bindings.workflow_0_2_5.binding.ACTION]
    )
  }
}
