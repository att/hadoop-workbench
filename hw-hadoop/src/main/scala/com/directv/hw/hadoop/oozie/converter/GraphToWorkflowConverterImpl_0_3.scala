...
class GraphToWorkflowConverterImpl_0_3 extends GraphToWorkflowConverterBase with LazyLogging {
  ...
  override lazy val pluginDeserializers: List[(Class[_], Class[_])] =
    com.directv.hw.hadoop.oozie.bindings.shell_action_0_3.pluginDescription.additionalDeserializers ++
      ...
      com.directv.hw.hadoop.oozie.bindings.workflow_0_3.pluginDescription.additionalDeserializers ++
  ...
  case class startProps (credential: java.util.List[CREDENTIAL])


  override protected def toWorkflow(graph: WorkflowGraph, options: ConversionOptions): supportedType = {
    ...
    graph.nodes foreach { node =>
      ...
      (node.`type`, node.subtype, node.version) match {
        case (Oozie.types.workflowControl, Oozie.subtypes.start, getSupportedWorkflowVersion) =>
          ...
          val startProperties = new Gson().fromJson(json, classOf[startProps])
          Option(startProperties) flatMap(s => Option(s.credential)) withFilter(!_.isEmpty) foreach { jsonCredentials =>
        ...
        case (Oozie.types.workflowControl, "fork", getSupportedWorkflowVersion) =>
          ...
          connections.filter(c => c.from == node.id) foreach {connection =>
