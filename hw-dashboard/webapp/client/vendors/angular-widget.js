...
angular.module("angularWidget", ["angularWidgetInternal"]).config(["$provide", "$injector", function ($provide, $injector) {
    ...
    $provide.decorator("$rootScope", ["$delegate", "$injector", function ($delegate, $injector) {
        ...
        $delegate.$digest = function () {
            var digestReturnValue = originalDigest.apply(this, arguments);
            $injector.invoke(["widgets", function (widgets) {
                widgets.notifyWidgets();
            }]);
            return digestReturnValue;
        };
        return $delegate;
    }]);
}]);
...
angular.module("angularWidgetInternal").provider("widgets", function () {
    ...
    this.$get = ["$injector", "$rootScope", function ($injector, $rootScope) {
        ...
        return {
            ...
            notifyWidget: function (widgetIndex) {
                var args = arguments;
                var widgetInjector = widgets[widgetIndex];
                if (widgetInjector) {
                    Array.prototype.splice.call(args, 0, 1);
                    return notifyInjector(widgetInjector, args);
                }
            },
